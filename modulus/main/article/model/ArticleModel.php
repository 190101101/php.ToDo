...
class ArticleModel extends model
{
    ...
    {
        return $this->db->t1count('article',"user_id=? ORDER BY article_id DESC", [User::user_id()])->count;
    }    

    public function ArticleList($start, $limit)
    {
        return $this->db->t1where('article', "user_id=?
    ...
    public function ArticleCreate()
    {
        ...
        $v->rule('lengthMin', 'article_title', 2);
        $v->rule('lengthMin', 'article_text', 2);
