...
class ArticleModel extends model
{
    ...
    {
        return $this->db->t2count('article', 'user', "article_id > 0 ORDER BY article_id DESC", [])->count;
    }    

    public function ArticleList($start, $limit)
    {
        return $this->db->t2where('article', 'user', "article_id > 0
            ORDER BY article_id DESC LIMIT {$start}, {$limit}", [], 2, 2);
    }

    #
    public function ByUserCount($user, $article)
    {
        return $this->db->t2count('article', 'user',
            "user.user_id=? && article.article_id=?", [$user, $article])->count;
    }    

    public function ByUserList($user, $article, $start, $limit)
    {
        return $this->db->t2where('article', 'user', "user.user_id=? && article.article_id=?
            ORDER BY article_id DESC LIMIT {$start}, {$limit}", [$user, $article], 2, 2);
    }
