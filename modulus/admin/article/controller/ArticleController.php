...
class ArticleController extends controller
{
    ...
    public function ByUser($user, $article)
    {
        $this->layout('panel', 'admin', 'article', 'ByUser', [
            'page' => $p = $this->page->page($this->article->ByUserCount($user, $article), 5),
            'article' => $this->article->ByUserList($user, $article, $p->start, $p->limit),
            'column' => $this->article->ArticleColumn(),
        ]);
    }
